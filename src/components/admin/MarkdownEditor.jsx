...
function MarkdownEditor({ value, onChange }) {
  ...
  return (
    <div className="border border-ivory/10 rounded-2xl overflow-hidden bg-slate/20">
      <div className="flex items-center justify-between px-4 py-3 border-b border-ivory/10 bg-slate/30">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${
              splitView ? 'bg-champagne/20 text-champagne' : 'text-slate hover:text-ivory'
            }`}
          >
            Split
          </button>
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${
              showPreview ? 'bg-champagne/20 text-champagne' : 'text-slate hover:text-ivory'
            }`}
          >
      ...
      <div className={`grid ${splitView ? 'grid-cols-2' : showPreview ? 'grid-cols-1' : 'grid-cols-1'}`}>
        <div className={`${splitView ? 'border-r border-ivory/10' : ''} ${!showPreview ? 'hidden' : ''}`}>
          <textarea
            ref={textareaRef}
            value={value}
            onChange={(e) => onChange(e.target.value)}
            onKeyDown={handleKeyDown}
            className="w-full h-80 lg:h-96 p-4 bg-transparent text-ivory resize-none focus:outline-none font-mono text-sm"
        ...
        {showPreview && (
          <div
            ref={previewRef}
            className="h-80 lg:h-96 p-4 overflow-y-auto prose prose-invert prose-sm max-w-none"
          >
            <ReactMarkdown className="text-ivory/80">
